...
async def update_synced_status(
    ...
    updated = db.query(Item).filter(
        ...
        {Item.last_sync_from_manager: datetime.utcnow()},
...
@router.get("/laundry-contracts", response_model=List[LaundryContractResponse])
async def get_laundry_contracts(db: Session = Depends(get_manager_db)):
...
@router.post("/laundry-contract/{contract_id}/finalize")
...
@router.post("/laundry-contract/{contract_id}/returned")
...
@router.post("/laundry-contract/{contract_id}/reactivate")
