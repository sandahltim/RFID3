...
class CSVImportService:
    ...
    def _import_scorecard_batch(self, df: pd.DataFrame) -> int:
        """Import scorecard trends batch with proper column mapping"""
        imported_count = 0
        session = self.Session()
        
        try:
            # Column mapping to match actual database structure
            column_mapping = {
                'week_ending_sunday': 'week_ending_sunday',
                'total_weekly_revenue': 'total_weekly_revenue', 
                '3607_revenue': 'col_3607_revenue',
                '6800_revenue': 'col_6800_revenue',
                '728_revenue': 'col_728_revenue',
                '8101_revenue': 'col_8101_revenue',
                'new_open_contracts_3607': 'new_open_contracts_3607',
                'new_open_contracts_6800': 'new_open_contracts_6800', 
                'new_open_contracts_728': 'new_open_contracts_728',
                'new_open_contracts_8101': 'new_open_contracts_8101',
                'deliveries_scheduled_next_7_days_wedstues_8101': 'deliveries_scheduled_next_7_days_weds_tues_8101',
                'on_reservation_next_14_days_3607': 'on_reservation_next_14_days_3607',
                'on_reservation_next_14_days_6800': 'on_reservation_next_14_days_6800',
                'on_reservation_next_14_days_728': 'on_reservation_next_14_days_728', 
                'on_reservation_next_14_days_8101': 'on_reservation_next_14_days_8101',
                'total_on_reservation_3607': 'total_on_reservation_3607',
                'total_on_reservation_6800': 'total_on_reservation_6800',
                'total_on_reservation_728': 'total_on_reservation_728',
                'total_on_reservation_8101': 'total_on_reservation_8101',
                'total_ar_45_days': 'total_ar_45_days',
                'total_discount_company_wide': 'total_discount_company_wide',
                'week_number': 'week_number',
                'open_quotes_8101': 'open_quotes_8101',
                'total_ar_cash_customers': 'total_ar_cash_customers',
                'column1': 'column1'
            }
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    for csv_col in df.columns:
                        clean_csv_col = self._clean_column_name(csv_col)
                        db_col = column_mapping.get(clean_csv_col)
                        
                        if db_col:
                            value = row[csv_col]
                            if pd.isna(value) or str(value).strip() == '':
                                record_data[db_col] = None
                            elif db_col == 'week_ending_sunday':
                                # Handle datetime column
                                if isinstance(value, (pd.Timestamp, datetime)):
                                    record_data[db_col] = value
                                else:
                                    try:
                                        record_data[db_col] = pd.to_datetime(value)
                                    except:
                                        record_data[db_col] = None
                            elif db_col == 'total_ar_45_days' and isinstance(value, str) and '%' in str(value):
                                # Handle percentage values - remove % and convert to decimal
                                try:
                                    numeric_val = float(str(value).replace('%', '').strip())
                                    record_data[db_col] = numeric_val
                                except:
                                    record_data[db_col] = None
                            elif db_col == 'week_number':
                                # Handle week number - should be integer
                                try:
                                    if isinstance(value, (pd.Timestamp, datetime)):
                                        # If it's a date, skip it or extract week number
                                        record_data[db_col] = None
                                    else:
                                        record_data[db_col] = int(float(str(value))) if pd.notna(value) and str(value).strip() != '' else 0
                                except:
                                    record_data[db_col] = None
                            else:
                                ...
                                try:
                                    if isinstance(value, (pd.Timestamp, datetime)):
                                        ...
                                        record_data[db_col] = float(value) if pd.notna(value) and str(value).strip() != '' else 0.0
                                except:
                                    record_data[db_col] = 0.0
                    
                    # Add required metadata fields
                    record_data['import_batch_id'] = 1  # Default batch ID
                    record_data['created_at'] = datetime.now()
                    
                    # Only include columns that exist in the database
                    valid_columns = [col for col in record_data.keys() if col in column_mapping.values() or col in ['import_batch_id', 'created_at']]
                    filtered_data = {col: record_data[col] for col in valid_columns}
                    ...
                    session.execute(insert_sql, filtered_data)
                    imported_count += 1
            ...
            session.commit()
            logger.info(f"Successfully imported {imported_count} scorecard records")
