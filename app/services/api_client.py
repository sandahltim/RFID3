...
class APIClient:
    ...
    def authenticate(self):
        ...
        for attempt in range(5):
            try:
                ...
                data = response.json()
                logger.debug(
                    f"Token attempt {attempt + 1}: Status {response.status_code}"
                )
                if response.status_code == 200 and data.get("result"):
                    ...
                    logger.error(
                        f"Token attempt {attempt + 1} failed: {response.status_code}"
                    )
                    if attempt < 4:
                        time.sleep(3)
                ...
                logger.error(
                    f"Token attempt {attempt + 1} failed: {str(e)}", extra=error_details
                )
                if attempt < 4:
                    time.sleep(3)
                else:
        ...
        raise APIException(
            "Failed to fetch access token after 5 attempts",
    ...
    def _make_request(
        ...
        if method == "GET":
            ...
            params["limit"] = min(
                requested_limit, 200
            )  # API caps responses at 200 records
        ...
        if method == "GET":
            ...
            while True:
                ...
                try:
                    ...
                    if response.status_code == 500:
                        ...
                        raise Exception(
                            f"500 Internal Server Error: {data.get('result', {}).get('message', 'Unknown error')}"
                        )
                    ...
                    logger.debug(
                        f"Fetched {len(records)} records, Total: {total_count}, Offset: {offset}"
                    )
                    all_data.extend(records)
                    if (
                        len(records) < params["limit"]
                        or offset + len(records) >= total_count
                    ):
                        break
            ...
            lock_context = (
                cache.lock("user_operation_lock", timeout=30)
                if user_operation
                else nullcontext()
            )
            try:
        ...
        if since_date and full_refresh:
            ...
            if since_dt:
                ...
                all_data = [
                    ...
                    if item.get("date_last_scanned") is None
                    or (
                        self.validate_date(
                            item.get("date_last_scanned"), "date_last_scanned"
                        )
                ...
                logger.info(
                    f"Filtered to {len(all_data)} items locally after fetching all"
                )
        return all_data
