...
class ScheduledSnapshotService:
    ...
    def create_weekly_snapshots(max_days_back=7):
        ...
        try:
            ...
            active_contracts = session.query(
            ...
            logger.info(f"Found {len(active_contracts)} active contracts in last {max_days_back} days")
            ...
            for contract_number, client_name in active_contracts:
                try:
                    ...
                    item_count = session.query(func.count(ItemMaster.tag_id)).filter(
                    ...
                    if item_count > 0:
                        ...
                        logger.info(f"Created snapshot for contract {contract_number}: {snapshot_count} items")
                    else:
                        logger.warning(f"Skipped contract {contract_number}: No items found")
                        results['contracts_processed'].append({
                            'contract_number': contract_number,
                            'client_name': client_name,
                            'items_count': 0,
            ...
            logger.info(f"Weekly snapshots completed: {results['successful_snapshots']} successful, "
            ...
            logger.error(f"Error in weekly snapshot automation: {str(e)}", exc_info=True)
    ...
    def cleanup_old_periodic_snapshots(days_to_keep=30):
        ...
        """
        logger.info(f"Starting cleanup of periodic snapshots older than {days_to_keep} days...")
        ...
        try:
            ...
            deleted_count = session.query(ContractSnapshot).filter(
            ...
            if session:
                session.rollback()
            logger.error(f"Error cleaning up periodic snapshots: {str(e)}", exc_info=True)
            raise
        finally:
            if session:
                session.close()
    ...
    def get_snapshot_schedule_info():
        ...
        try:
            ...
            last_periodic = session.query(
                func.max(ContractSnapshot.snapshot_date)
            ).filter(
                ContractSnapshot.snapshot_type == 'periodic'
            ).scalar()
            ...
            recent_count = session.query(func.count(ContractSnapshot.id)).filter(
                ContractSnapshot.snapshot_type == 'periodic',
                ContractSnapshot.snapshot_date >= week_ago
            ).scalar()
            
            return {
                'last_periodic_snapshot': last_periodic.isoformat() if last_periodic else None,
