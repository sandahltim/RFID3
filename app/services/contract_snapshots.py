...
class ContractSnapshotService:
    ...
    @staticmethod
    def create_contract_snapshot(contract_number, snapshot_type='manual', created_by='system'):
        """
        ...
        try:
            ...
            client_info = session.query(Transaction.client_name).filter(
            ...
            session.commit()
            logger.info(f"Created {snapshot_count} snapshots for contract {contract_number}, type: {snapshot_type}")
            return snapshot_count
            
        except Exception as e:
            if session:
                session.rollback()
            logger.error(f"Error creating contract snapshot for {contract_number}: {str(e)}", exc_info=True)
            raise
        finally:
            if session:
                session.close()
    ...
    def get_contract_snapshot(contract_number, snapshot_date=None):
        ...
        try:
            ...
            logger.error(f"Error getting contract snapshot for {contract_number}: {str(e)}", exc_info=True)
            raise
        finally:
            if session:
                session.close()
    ...
    def cleanup_old_snapshots(days_to_keep=365):
        ...
        try:
            ...
            deleted_count = session.query(ContractSnapshot).filter(
                ...
                ContractSnapshot.snapshot_type.in_(['periodic', 'status_change'])  # Keep manual and contract start/end
            ).delete()
            
            session.commit()
            logger.info(f"Cleaned up {deleted_count} old contract snapshots older than {days_to_keep} days")
    ...
    def get_contract_history_with_snapshots(contract_number):
        ...
        try:
            ...
            current_items = session.query(ItemMaster).filter(
                ItemMaster.last_contract_num == contract_number
            ).all()
            ...
            logger.error(f"Error getting contract history with snapshots for {contract_number}: {str(e)}", exc_info=True)
            raise
        finally:
            if session:
                session.close()