    ...
    try:
        ...
        for endpoint, times in performance_metrics['request_times'].items():
            if times:
                ...
                recent_avg = sum(entry['time'] for entry in recent_times) / len(recent_times) if recent_times else 0
                ...
                endpoint_stats[endpoint] = {
                    ...
                    'error_rate': (performance_metrics['error_counts'][endpoint] / 
                                 performance_metrics['endpoint_calls'][endpoint] * 100) if performance_metrics['endpoint_calls'][endpoint] > 0 else 0
                }
        ...
        try:
            ...
            redis_info = cache.connection_pool.connection_kwargs if hasattr(cache, 'connection_pool') else {}
...
def get_performance_summary():
    ...
    try:
        ...
        return {
            'status': 'healthy' if error_rate < 5 and avg_response_time < 1000 else 'degraded',
            'total_requests': total_requests,
            'error_rate': round(error_rate, 2),
            'avg_response_time': round(avg_response_time, 2),
            'cpu_percent': cpu_percent,
            'memory_percent': memory_percent,
            'uptime_minutes': (datetime.now() - performance_metrics['last_reset']).total_seconds() / 60
        }
    except Exception as e:
