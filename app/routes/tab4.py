...
# Version marker
logger.info(
    "Deployed tab4.py version: 2025-05-06-v36 at %s",
...
def tab4_view():
    ...
    try:
        session = db.session()
        logger.info(
            "Starting new session for tab4 at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        ...
        total_items_inventory_dict = {
            ...
            for item in total_items_inventory_query
        }
        logger.info(
            f"Total items inventory for ItemMaster contracts: {total_items_inventory_dict} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )

        # Step 3: Fetch the latest transaction details for these contracts
        logger.info(
            "Fetching latest transaction details for ItemMaster contracts at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        latest_transactions = {}
        if contract_numbers_from_items:
            transaction_query = (
                ...
                .order_by(Transaction.contract_number, desc(Transaction.scan_date))
        ...
        logger.info(
            f"Latest transactions for ItemMaster contracts: {latest_transactions} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )

        # Step 4: Fetch hand-counted items for contracts starting with 'L'
        logger.info(
            "Fetching hand-counted items for contracts starting with 'L' at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        # Fetch all contracts starting with 'L' from HandCountedItems
        hand_counted_contract_numbers = (
            session.query(HandCountedItems.contract_number)
            .filter(
        ...
        hand_counted_contract_numbers = [
            c.contract_number for c in hand_counted_contract_numbers
        ]
        ...
        added_quantities = (
            ...
            .group_by(HandCountedItems.contract_number)
            .all()
        )

        added_quantities_dict = {
            item.contract_number: item.added_quantity for item in added_quantities
        }
        ...
        removed_quantities_dict = {
            item.contract_number: item.removed_quantity for item in removed_quantities
        }
        ...
        logger.info(
            f"Hand-counted contracts: {hand_counted_contracts} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        ...
        for contract in item_master_contracts:
            ...
            transaction_info = latest_transactions.get(
                contract_number, {"client_name": "N/A", "scan_date": "N/A"}
            )
            contracts_dict[contract_number] = {
                ...
                "total_items_inventory": total_items_inventory_dict.get(
                    contract_number, 0
                ),
        ...
        for contract_number, hand_counted_total in hand_counted_contracts:
            ...
            if contract_number in contracts_dict:
                ...
                latest_transaction = (
                    session.query(Transaction.client_name, Transaction.scan_date)
                    .filter(
                ...
                contracts_dict[contract_number] = {
                    "contract_number": contract_number,
                    "client_name": (
                        latest_transaction.client_name if latest_transaction else "N/A"
                    ),
                    "scan_date": (
                        latest_transaction.scan_date.isoformat()
        ...
        for contract in contracts_dict.values():
            ...
            if (
                total_items_on_contract == 0
                and total_items_inventory == 0
                and not has_hand_counted_history
            ):
                logger.info(
                    f"Skipping contract {contract['contract_number']}: No items, no inventory, no hand-counted history at %s",
                    datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                )
                continue
        ...
        completed_status_records = (
            session.query(LaundryContractStatus)
            .filter(
        ...
        if contract_numbers:
            status_query = (
                session.query(LaundryContractStatus)
                .filter(LaundryContractStatus.contract_number.in_(contract_numbers))
                .all()
            )
            ...
            for contract in contracts:
                ...
                if status_info:
                    ...
                    contract["finalized_date"] = (
                        status_info.finalized_date.isoformat()
                        if status_info.finalized_date
                        else None
                    )
                    contract["returned_date"] = (
                        status_info.returned_date.isoformat()
                        if status_info.returned_date
                        else None
                    )
        ...
        logger.info(
            f"Fetched {len(contracts)} contracts for tab4: {[c['contract_number'] for c in contracts]} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.info(
            f"Fetched {len(contracts)} contracts for tab4: {[c['contract_number'] for c in contracts]} at %s",
        ...
        return render_template(
            "tab4.html", contracts=contracts, cache_bust=int(time.time())
        )
    except Exception as e:
        logger.error(
            f"Error rendering Tab 4: {str(e)} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            exc_info=True,
        )
...
def hand_counted_contracts():
    ...
    try:
        ...
        contracts = (
            session.query(ItemMaster.last_contract_num)
            .filter(
            ...
            .group_by(ItemMaster.last_contract_num)
            .having(func.count(ItemMaster.tag_id) > 0)
            .order_by(ItemMaster.last_contract_num)
            .all()
        )
        ...
        logger.error(
            f"Error fetching hand-counted contracts: {str(e)} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            exc_info=True,
        )
...
def hand_counted_items_by_contract():
    ...
    try:
        ...
        items = (
            ...
            .group_by(HandCountedItems.item_name)
            .all()
        )
        ...
        if not item_list and contract_number:
            added_items = (
                session.query(HandCountedItems.item_name)
                .filter(
                    HandCountedItems.contract_number == contract_number,
                    HandCountedItems.action == "Added",
                )
            ...
            for item in added_items:
                ...
                added_qty = (
                    session.query(func.sum(HandCountedItems.quantity))
                    .filter(
                        HandCountedItems.contract_number == contract_number,
                ...
                if (
                    added_qty > 0
                ):  # Include items that have been added, even if fully removed
                    item_list.append({"item_name": item_name, "quantity": net_qty})

        logger.info(
            f"Returned hand-counted items for contract {contract_number}: {item_list} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        return jsonify({"items": item_list})
    except Exception as e:
        logger.error(
            f"Error fetching hand-counted items for contract {contract_number}: {str(e)} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            exc_info=True,
        )
        return jsonify({"error": str(e)}), 500
...
def hand_counted_catalog():
    ...
    try:
        ...
        try:
            items = session.query(
                HandCountedCatalog.item_name, HandCountedCatalog.hand_counted_name
            ).all()
        except ProgrammingError:
            session.rollback()
            logger.warning("hand_counted_catalog table missing; returning empty list")
            current_app.logger.warning(
                "hand_counted_catalog table missing; returning empty list"
            )
            items = []
        ...
        for item in items:
            display_name = (
                item.hand_counted_name if item.hand_counted_name else item.item_name
            )
            item_list.append(display_name)
        logger.info(
            f"Returned hand-counted catalog items: {item_list} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        return jsonify({"items": item_list})
    except Exception as e:
        logger.error(
            f"Error fetching hand-counted catalog: {str(e)} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            exc_info=True,
        )
        return jsonify({"error": str(e)}), 500
...
def hand_counted_catalog_categorized():
    ...
    try:
        ...
        try:
            ...
            items_with_categories = (
                ...
                .outerjoin(
                    ...
                    HandCountedCatalog.rental_class_id
            ...
            session.rollback()
            logger.warning(
                "hand_counted_catalog table missing; returning empty categorized list"
            )
            current_app.logger.warning(
                "hand_counted_catalog table missing; returning empty categorized list"
            )
        ...
        logger.info(
            f"Returned categorized hand-counted catalog: {len(categorized_items)} categories"
        )
        return jsonify({"categories": categorized_items})

    except Exception as e:
        logger.error(
            f"Error fetching categorized hand-counted catalog: {str(e)}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
    finally:
        if session:
...
def contract_items_count():
    ...
    try:
        ...
        logger.info(
            f"Returned contract items count for {contract_number}: {total_items} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        return jsonify({"total_items": total_items})
    except Exception as e:
        logger.error(
            f"Error calculating items on contract for {contract_number}: {str(e)} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            exc_info=True,
        )
        return jsonify({"error": str(e)}), 500
...
def next_contract_number():
    ...
    try:
        session = db.session()
        latest_item = (
            session.query(func.max(ItemMaster.last_contract_num))
            .filter(func.upper(ItemMaster.last_contract_num).like("L%"))
            .scalar()
        )
        latest_hand = (
            session.query(func.max(HandCountedItems.contract_number))
            .filter(func.upper(HandCountedItems.contract_number).like("L%"))
            .scalar()
        )
        ...
        logger.error(
            f"Error calculating next contract number: {str(e)} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            exc_info=True,
        )
        return jsonify({"error": str(e)}), 500
...
def hand_counted_entries():
    ...
    try:
        ...
        logger.info(
            f"Returned hand-counted entries for {contract_number}: {hand_counted_total} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        return jsonify({"hand_counted_entries": hand_counted_total})
    except Exception as e:
        logger.error(
            f"Error calculating hand-counted entries for {contract_number}: {str(e)} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            exc_info=True,
        )
        return jsonify({"error": str(e)}), 500
...
def tab4_common_names():
    ...
    logger.info(
        f"Fetching common names for contract_number={contract_number}, page={page}, sort={sort}, fetch_all={fetch_all} at %s",
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    )
    current_app.logger.info(
        f"Fetching common names for contract_number={contract_number}, page={page}, sort={sort}, fetch_all={fetch_all} at %s",
    ...
    if not contract_number:
        logger.error(
            "Missing required parameter: contract_number is required at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.error(
            "Missing required parameter: contract_number is required at %s",
    ...
    try:
        ...
        common_names_query = (
            ...
            .group_by(ItemMaster.common_name)
        )

        common_names_all = common_names_query.all()

        logger.info(
            f"Common names from id_item_master for contract {contract_number}: {[(name, count) for name, count in common_names_all]} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.info(
            f"Common names from id_item_master for contract {contract_number}: {[(name, count) for name, count in common_names_all]} at %s",
        ...
        removed_dict = {
            item.common_name: item.removed_quantity for item in hand_counted_removed
        }
        ...
        logger.info(
            f"Hand-counted items as common names for contract {contract_number}: {hand_counted_items} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.info(
            f"Hand-counted items as common names for contract {contract_number}: {hand_counted_items} at %s",
        ...
        for name, on_contracts in common_names_all:
            ...
            total_items_inventory = (
                session.query(func.count(ItemMaster.tag_id))
                .filter(ItemMaster.common_name == name)
                .scalar()
            )
            common_names[name] = {
                "name": name,
            ...
            try:
                ...
                if field in ["name", "on_contracts", "total_items_inventory"]:
                    common_names_list.sort(
                        key=lambda x: (
                            x[field].lower() if isinstance(x[field], str) else x[field]
                        ),
                        reverse=reverse,
                    )
            except ValueError:
        ...
        logger.info(
            f"Returning {len(paginated_common_names)} common names for contract {contract_number} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.info(
            f"Returning {len(paginated_common_names)} common names for contract {contract_number} at %s",
        ...
        logger.error(
            f"Error fetching common names for contract {contract_number}: {str(e)} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.error(
            f"Error fetching common names for contract {contract_number}: {str(e)} at %s",
...
def tab4_data():
    ...
    logger.info(
        f"Fetching items for contract_number={contract_number}, common_name={common_name}, page={page}, sort={sort} at %s",
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    )
    current_app.logger.info(
        f"Fetching items for top level contract_number={contract_number}, common_name={common_name}, page={page}, sort={sort} at %s",
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    )

    if not contract_number or not common_name:
        logger.error(
            "Missing required parameters: contract number and common name are required at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.error(
            "Missing required parameters: contract number and common name are required at %s",
    ...
    try:
                ...
                items_data.append(
                    {
                        ...
                        "last_scanned_date": (
                            item.timestamp.isoformat() if item.timestamp else "N/A"
                        ),
                    }
                )
        else:
        ...
        logger.info(
            f"Items for contract {contract_number}, common_name {common_name}: {len(items_data)} items at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.info(
            f"Items for contract {contract_number}, common_name {common_name}: {len(items_data)} items at %s",
        ...
        logger.error(
            f"Error fetching items for contract {contract_number}, common_name {common_name}: {str(e)} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.error(
            f"Error fetching items for contract {contract_number}, common_name {common_name}: {str(e)} at %s",
...
def contract_history_print():
    ...
    try:
        ...
        total_hand_counted = sum(
            item_tuple.HandCountedItems.quantity
            for item_tuple in hand_counted_items
            if item_tuple.HandCountedItems.action == "Added"
        )
        total_hand_counted -= sum(
            item_tuple.HandCountedItems.quantity
            for item_tuple in hand_counted_items
            if item_tuple.HandCountedItems.action == "Removed"
        )
        ...
        contract_data = {
            ...
            "start_date": (
                contract_info.start_date.strftime("%Y-%m-%d %H:%M")
                if contract_info.start_date
                else "N/A"
            ),
            "last_activity": (
                contract_info.last_activity.strftime("%Y-%m-%d %H:%M")
                if contract_info.last_activity
                else "N/A"
            ),
            ...
            "rfid_items": [
                {
                    ...
                    "last_scanned": (
                        item.date_last_scanned.strftime("%Y-%m-%d %H:%M")
                        if item.date_last_scanned
                        else "N/A"
                    ),
                    "notes": item.notes or "",
            ...
            "hand_counted_items": [
                {
                    "item_name": (
                        item_tuple.HandCountedCatalog.hand_counted_name
                        if item_tuple.HandCountedCatalog
                        and item_tuple.HandCountedCatalog.hand_counted_name
                        else item_tuple.HandCountedItems.item_name
                    ),
                    "quantity": item_tuple.HandCountedItems.quantity,
                    "action": item_tuple.HandCountedItems.action,
                    "timestamp": (
                        item_tuple.HandCountedItems.timestamp.strftime("%Y-%m-%d %H:%M")
                        if item_tuple.HandCountedItems.timestamp
                        else "N/A"
                    ),
            ...
            "recent_activity": [
                {
                    ...
                    "scan_date": (
                        t.scan_date.strftime("%Y-%m-%d %H:%M") if t.scan_date else "N/A"
                    ),
        ...
        response.headers["Content-Disposition"] = (
            f"inline; filename=contract_{contract_number}_history.html"
        )
        return response

    except Exception as e:
        logger.error(
            f"Error generating contract history print: {str(e)}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
    finally:
        if session:
    ...
    try:
        ...
        if not hand_counted_items:
            return (
                jsonify(
                    {
                        "error": f"No hand counted items found for contract {contract_number}"
                    }
                ),
                404,
            )
        ...
        for item_tuple in hand_counted_items:
            ...
            item_summary[display_name]["net"] = (
                item_summary[display_name]["added"]
                - item_summary[display_name]["removed"]
            )

            if (
                not item_summary[display_name]["last_activity"]
                or item.timestamp > item_summary[display_name]["last_activity"]
            ):
            ...
            "chronological_history": [
                {
                    ...
                    "action": item_tuple.HandCountedItems.action,
                    "timestamp": (
                        item_tuple.HandCountedItems.timestamp.strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
                        if item_tuple.HandCountedItems.timestamp
                        else "N/A"
        ...
        response.headers["Content-Type"] = "text/html"
        response.headers["Content-Disposition"] = (
            f"inline; filename=hand_counted_history_{contract_number}.html"
        )
        return response

    except Exception as e:
        logger.error(
            f"Error generating hand counted history print: {str(e)}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
    finally:
        if session:
...
def contract_items_print():
    ...
    try:
        ...
        snapshot_items = (
            ...
            .filter(ContractSnapshot.contract_number == contract_number)
            .order_by(ContractSnapshot.common_name, ContractSnapshot.tag_id)
            .all()
        )
        ...
        if snapshot_items:
            ...
            logger.info(
                f"Using {len(snapshot_items)} snapshot items for contract {contract_number}"
            )
        else:
            ...
            items = (
                session.query(ItemMaster)
                .filter(ItemMaster.last_contract_num == contract_number)
                .order_by(ItemMaster.common_name, ItemMaster.tag_id)
                .all()
            )
            logger.info(
                f"Using {len(items)} current items for contract {contract_number} (no snapshots found)"
            )
        ...
        items_data = {
            ...
            "start_date": (
                ...
                if date_info and date_info.start_date
                else "N/A"
            ),
            "last_activity": (
                date_info.last_activity.strftime("%Y-%m-%d")
                if date_info and date_info.last_activity
                else "N/A"
            ),
        ...
        response.headers["Content-Type"] = "text/html"
        response.headers["Content-Disposition"] = (
            f"inline; filename=contract_items_{contract_number}.html"
        )
        return response
...
def tab4_hand_counted_items():
    ...
    logger.info(
        f"Fetching hand-counted items for contract_number={contract_number} at %s",
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    )
    current_app.logger.info(
        f"Fetching hand-counted items for contract_number={contract_number} at %s",
    ...
    try:
        session = db.session()
        query = session.query(HandCountedItems).order_by(
            HandCountedItems.timestamp.desc()
        )
        if contract_number:
        ...
        logger.info(
            f"Found {len(items)} hand-counted items for contract {contract_number} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.info(
            f"Found {len(items)} hand-counted items for contract {contract_number} at %s",
...
def add_hand_counted_item():
    ...
    logger.info(
        f"Adding hand-counted item: contract_number={contract_number}, item_name={item_name}, quantity={quantity}, action={action}, employee_name={employee_name} at %s",
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    )
    current_app.logger.info(
        f"Adding hand-counted item: contract_number={contract_number}, item_name={item_name}, quantity={quantity}, action={action}, employee_name={employee_name} at %s",
    ...
    if not all([contract_number, item_name, quantity, action, employee_name]):
        logger.error(
            "Missing required fields for adding hand-counted item at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.error(
            "Missing required fields for adding hand-counted item at %s",
    ...
    try:
        ...
        if quantity <= 0:
            logger.error(
                "Quantity must be a positive integer at %s",
                datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            )
            current_app.logger.error(
                "Quantity must be a positive integer at %s",
        ...
        logger.error(
            "Quantity must be a valid integer at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.error(
            "Quantity must be a valid integer at %s",
    ...
    try:
        ...
        session.commit()
        logger.info(
            f"Successfully added hand-counted item for contract {contract_number} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.info(
            f"Successfully added hand-counted item for contract {contract_number} at %s",
        ...
        return jsonify(
            {
                "message": f"Successfully added {quantity} {item_name} to {contract_number}"
            }
        )
    except Exception as e:
        logger.error(
            f"Error adding hand-counted item: {str(e)} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.error(
            f"Error adding hand-counted item: {str(e)} at %s",
...
def remove_hand_counted_item():
    ...
    logger.info(
        f"Removing hand-counted item: contract_number={contract_number}, item_name={item_name}, quantity={quantity}, action={action}, employee_name={employee_name} at %s",
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    )
    current_app.logger.info(
        f"Removing hand-counted item: contract_number={contract_number}, item_name={item_name}, quantity={quantity}, action={action}, employee_name={employee_name} at %s",
    ...
    if not all([contract_number, item_name, quantity, action, employee_name]):
        logger.error(
            "Missing required fields for removing hand-counted item at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.error(
            "Missing required fields for removing hand-counted item at %s",
    ...
    try:
        ...
        logger.info(
            f"Current quantity for {contract_number}/{item_name}: Added={added_quantity}, Removed={removed_quantity}, Net={current_quantity} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        ...
        if current_quantity < quantity:
            logger.info(
                f"Cannot remove {quantity} items from {contract_number}/{item_name}: current_quantity={current_quantity} at %s",
                datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            )
            current_app.logger.info(
                f"Cannot remove {quantity} items from {contract_number}/{item_name}: current_quantity={current_quantity} at %s",
            ...
            return (
                jsonify(
                    {
                        "error": f"Cannot remove {quantity} items from {contract_number}/{item_name}. Quantity would be negative."
                    }
                ),
                400,
            )
        ...
        session.commit()
        logger.info(
            f"Successfully removed {quantity} hand-counted items for contract {contract_number}, item {item_name} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.info(
            f"Successfully removed {quantity} hand-counted items for contract {contract_number}, item {item_name} at %s",
        ...
        return jsonify(
            {
                "message": f"Successfully removed {quantity} {item_name} from {contract_number}"
            }
        )
    except Exception as e:
        logger.error(
            f"Error removing hand-counted item: {str(e)} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.error(
            f"Error removing hand-counted item: {str(e)} at %s",
...
def full_items_by_rental_class():
    ...
    if not contract_number or not common_name:
        logger.error(
            "Category (contract_number) and common name are required at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.error(
            "Category (contract_number) and common name are required at %s",
        ...
        return (
            jsonify(
                {"error": "Category (contract_number) and common name are required"}
            ),
            400,
        )
    ...
    try:
        ...
        logger.error(
            f"Error fetching full items for contract {contract_number}, common_name {common_name}: {str(e)} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.error(
            f"Error fetching full items for contract {contract_number}, common_name {common_name}: {str(e)} at %s",
...
def get_contract_date():
    ...
    try:
        ...
        logger.error(
            f"Error fetching contract date for {contract_number}: {str(e)} at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        current_app.logger.error(
            f"Error fetching contract date for {contract_number}: {str(e)} at %s",
...
def finalize_contract():
    ...
    try:
        ...
        status_record = (
            session.query(LaundryContractStatus)
            .filter(LaundryContractStatus.contract_number == contract_number)
            .first()
        )
        ...
        if session:
            session.rollback()
        logger.error(
            f"Error finalizing contract {contract_number}: {str(e)}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
    finally:
        if session:
...
def mark_returned():
    ...
    try:
        ...
        if session:
            session.rollback()
        logger.error(
            f"Error marking contract {contract_number} as returned: {str(e)}",
            exc_info=True,
        )
        return jsonify({"error": str(e)}), 500
    finally:
    ...
    try:
        ...
        if status_record:
            ...
            if notes:
                existing_notes = status_record.notes or ""
                status_record.notes = (
                    f"{existing_notes}\nReactivated by {user}: {notes}"
                    if existing_notes
                    else f"Reactivated by {user}: {notes}"
                )
        else:
            ...
            status_record = LaundryContractStatus(
                ...
                status="active",
                notes=(
                    f"Reactivated by {user}: {notes}"
                    if notes
                    else f"Reactivated by {user}"
                ),
            )
            session.add(status_record)
        ...
        if session:
            session.rollback()
        logger.error(
            f"Error reactivating contract {contract_number}: {str(e)}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
    finally:
        if session:
