...
# Blueprint for Tab 3 - DO NOT MODIFY BLUEPRINT NAME
# Added on 2025-04-21 as a placeholder tab, mirrors Tab 2 structure without specific filters
# Currently displays all open contracts (same as Tab 2), can be modified for specific use cases later
...
def tab3_view():
    ...
    try:
        current_app.logger.info("Loading tab 3")
        ...
        current_app.logger.info(f"Fetched {len(categories)} contracts for tab 3")
...
@tab3_bp.route('/tab/3/categories')
def tab3_categories():
    # Route to render the categories table for Tab 3
    ...
    try:
        # Same query as tab3_view to fetch all open contracts
        contract_data = db.session.query(
            ItemMaster.last_contract_num,
            func.count(ItemMaster.tag_id).label('total_items'),
            func.max(Transaction.client_name).label('customer_name'),
        ...
        current_app.logger.error(f"Error fetching contracts for tab 3: {str(e)}", exc_info=True)
...
def tab3_common_names():
    # Route to fetch common names under a specific category for a contract
    try:
