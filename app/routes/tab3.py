...
def cache_get(key):
    ...
    try:
        ...
        logger.error(
            f"Error accessing Redis cache for key {key}: {str(e)}", exc_info=True
        )
        return None
...
def cache_set(key, value, timeout=60):
    ...
    try:
        ...
        logger.error(
            f"Error setting Redis cache for key {key}: {str(e)}", exc_info=True
        )
...
def tab3_view():
    ...
    try:
        ...
        in_service_statuses_sql = ", ".join(
            [f"'{status}'" for status in in_service_statuses]
        )
        ...
        if crew_filter:
            crew_categories = (
                TENT_CATEGORIES
                if crew_filter == "Tent Crew"
                else (
                    LINEN_CATEGORIES
                    if crew_filter == "Linen Crew"
                    else SERVICE_DEPT_CATEGORIES
                )
            )
            if crew_filter == "Service Dept":
                ...
                summary_query += (
                    " AND COALESCE(u.category, r.category, 'Other') IN :crew_categories"
                )
        ...
        logger.debug(
            f"Executing detail query: {detail_query} with params: {detail_params}"
        )
        ...
        if tag_ids:
            ...
            max_scan_date_subquery = (
                ...
                .filter(Transaction.tag_id.in_(tag_ids))
            ...
            for t in transactions:
                repair_types = []
                if t.dirty_or_mud:
                    repair_types.append("Dirty/Mud")
                if t.leaves:
                    repair_types.append("Leaves")
                if t.oil:
                    repair_types.append("Oil")
                if t.mold:
                    repair_types.append("Mold")
                if t.stain:
                    repair_types.append("Stain")
                if t.oxidation:
                    repair_types.append("Oxidation")
                if t.rip_or_tear:
                    repair_types.append("Rip/Tear")
                if t.sewing_repair_needed:
                    repair_types.append("Sewing Repair Needed")
                if t.grommet:
                    repair_types.append("Grommet")
                if t.rope:
                    repair_types.append("Rope")
                if t.buckle:
                    repair_types.append("Buckle")
                if t.wet:
                    repair_types.append("Wet")
                if t.other:
                    repair_types.append(f"Other: {t.other}")
                if t.service_required:
                    repair_types.append("Service Required")
                ...
                transaction_data[t.tag_id] = {
                    ...
                    "longitude": (
                        float(t.longitude) if t.longitude is not None else None
                    ),
                    "latitude": float(t.latitude) if t.latitude is not None else None,
                    "last_transaction_scan_date": (
                        t.scan_date.isoformat() if t.scan_date else "N/A"
                    ),
                }
        ...
        for item in items_in_service:
            ...
            if group:
                ...
                logger.debug(
                    f"Updated existing group: rental_class_id={item['rental_class_num']}, common_name={item['common_name']}"
                )
            else:
                unmatched_items.append(item_details)
                logger.debug(
                    f"Unmatched item: tag_id={item['tag_id']}, common_name={item['common_name']}"
                )
        ...
        for item in unmatched_items:
            ...
            logger.debug(
                f"Created new group for unmatched item: rental_class_id={item['rental_class_id']}, common_name={item['common_name']}, crew={crew}"
            )

        # Filter out groups with no in-service items
        for crew in crew_groups:
            crew_groups[crew] = [
                g for g in crew_groups[crew] if g["number_in_service"] > 0
            ]
        ...
        for crew, groups in crew_groups.items():
            ...
            for group in groups:
                logger.debug(
                    f"Group: rental_class_id={group['rental_class_id']}, common_name={group['common_name']}, items={len(group['item_list'])}, statuses={group['statuses']}, crew={group['crew']}"
                )

        # Fetch common names for dropdown
        common_names = (
            session.query(SeedRentalClass.common_name)
...
def tab3_common_names():
    ...
    try:
        ...
        total_items = (
            session.execute(
                text(count_query), {"rental_class_id": normalized_rental_class_id}
            ).scalar()
            or 0
        )
        ...
        logger.info(
            f"Fetched and cached {len(common_names)} common names for rental_class_id {normalized_rental_class_id}, total: {total_items}"
        )
...
def tab3_data():
    ...
    try:
        ...
        if not rental_class_id or not common_name:
            logger.warning(
                f"Missing parameters: rental_class_id={rental_class_id}, common_name={common_name}"
            )
            return (
                jsonify({"error": "rental_class_id and common_name are required"}),
                400,
            )
        ...
        normalized_rental_class_id = normalize_rental_class_id(rental_class_id)
        logger.debug(
            f"Normalized rental_class_id: {normalized_rental_class_id}, common_name: {common_name}"
        )

        # Check cache
        cache_key = (
            f"tab3_items_{normalized_rental_class_id}_{common_name}_{page}_{per_page}"
        )
        cached_data = cache_get(cache_key)
        if cached_data:
            logger.info(f"Returning cached data for {cache_key}")
            ...
            .filter(
                ...
                or_(
                    ItemMaster.status.in_(
                        ["Repair", "Needs to be Inspected", "Wash", "Wet"]
                    ),
        ...
        cache_set(cache_key, response, timeout=60)
        logger.info(
            f"Fetched and cached {len(items_data)} items for rental_class_id {normalized_rental_class_id}, common_name {common_name}, total: {total_items}"
        )
        return jsonify(response), 200
    except Exception as e:
...
def remove_csv_item():
    ...
    try:
        ...
        logger.info(
            f"AUDIT: CSV_ITEM_REMOVED - tag_id={tag_id}, operation=manual_removal, removed_from_print_queue=true, timestamp={datetime.now().isoformat()}"
        )
...
def sync_to_pc():
    ...
    try:
        ...
        for attempt in range(max_lock_attempts):
            try:
                ...
                if attempt == max_lock_attempts - 1:
                    logger.warning(
                        f"Could not acquire lock after {max_lock_attempts} attempts"
                    )
                    lock_file.close()
                    return (
                        jsonify(
                            {
                                "error": "System busy - another sync operation is in progress. Please try again."
                            }
                        ),
                        429,
                    )
                logger.debug(
                    f"Lock attempt {attempt + 1} failed, waiting {lock_wait_time}s"
                )
        ...
        if not common_name or not isinstance(quantity, int) or quantity <= 0:
            logger.warning(
                f"Invalid input: common_name={common_name}, quantity={quantity}"
            )
        ...
        if not os.access(SHARED_DIR, os.W_OK):
            ...
            return (
                jsonify({"error": f"Shared directory {SHARED_DIR} is not writable"}),
                500,
            )
        ...
        if os.path.exists(CSV_FILE_PATH):
            try:
                with open(CSV_FILE_PATH, "r") as csvfile:
                    ...
                    if not first_line:
                        ...
                        if not all(
                            ...
                            logger.warning(
                                "CSV file exists but lacks valid headers. Rewriting with headers."
                            )
                            needs_header = True
                        else:
                            for row in reader:
                ...
                logger.error(
                    f"Error reading existing CSV file: {str(e)}", exc_info=True
                )
                return (
                    jsonify({"error": f"Failed to read existing CSV file: {str(e)}"}),
                    500,
                )
        ...
        try:
            ...
            logger.debug(
                f"Found {len(sold_items)} 'Sold' items in ItemMaster for common_name={common_name}"
            )
            ...
            for item in sold_items:
                ...
                if item.tag_id in existing_tag_ids:
                    logger.debug(
                        f"Skipping tag_id {item.tag_id} as it’s already in CSV"
                    )
                    continue
                synced_items.append(
                    {
                        ...
                        "status": item.status,
                        "bin_location": item.bin_location,
                        "rental_class_num": normalize_rental_class_id(
                            item.rental_class_num
                        ),
                        "is_new": False,
                    }
                )
                ...
                logger.debug(
                    f"Added to synced_items: tag_id={item.tag_id}, common_name={item.common_name}, status={item.status}, rental_class_num={item.rental_class_num}"
                )
        except sqlalchemy.exc.DatabaseError as e:
            logger.error(f"Database error querying ItemMaster: {str(e)}", exc_info=True)
            return (
                jsonify({"error": f"Database error querying ItemMaster: {str(e)}"}),
                500,
            )
        ...
        if len(synced_items) < quantity:
            ...
            logger.info(
                f"Generating {remaining_quantity} new items for common_name={common_name}"
            )

            seed_entry = (
                session.query(SeedRentalClass)
                .filter(
            ...
            if not seed_entry:
                logger.warning(
                    f"No SeedRentalClass entry found for common_name={common_name} and bin_location in ['pack', 'resale']"
                )
                return (
                    jsonify(
                        {
                            "error": f"No SeedRentalClass entry found for common name '{common_name}'"
                        }
                    ),
                    404,
                )
            ...
            @retry(
                ...
                before_sleep=lambda retry_state: logger.debug(
                    f"Retrying INSERT due to lock timeout, attempt {retry_state.attempt_number}"
                ),
            )
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    max_tag_id = (
                        session.query(func.max(ItemMaster.tag_id))
                    ...
                    if (
                        max_tag_id
                        and len(max_tag_id) == 24
                        and max_tag_id.startswith("425445")
                    ):
                        try:
                            incremental_part = int(max_tag_id[6:], 16)
                            new_num = incremental_part + 1
                            logger.debug(
                                f"Starting tag generation from incremental_part: {incremental_part}, new_num: {new_num}"
                            )
                        except ValueError:
                            logger.warning(
                                f"Invalid incremental part in max_tag_id: {max_tag_id}, starting from 1"
                            )
                    ...
                    for i in range(remaining_quantity):
                        ...
                        ):  # Max 18 hex digits (72 bits)
                            raise ValueError(
                                "Tag ID sequence overflow - too many tags generated"
                            )
                        ...
                        if len(tag_id) != 24:
                            raise ValueError(
                                f"Generated tag_id {tag_id} must be 24 characters long, got {len(tag_id)}"
                            )
                    ...
                    if attempt == max_retries - 1:
                        raise e
                    logger.warning(
                        f"Tag ID generation attempt {attempt + 1} failed: {e}, retrying..."
                    )
            ...
            if len(new_tag_ids) != remaining_quantity:
                raise ValueError(
                    f"Failed to generate {remaining_quantity} unique tag IDs, only got {len(new_tag_ids)}"
                )
            ...
            with session.no_autoflush:
                for i, tag_id in enumerate(new_tag_ids):
                    ...
                    logger.info(
                        f"AUDIT: TAG_CREATED - tag_id={tag_id}, common_name={common_name}, rental_class_num={rental_class_num}, initial_status=Sold, created_by=sync_to_pc, timestamp={datetime.now().isoformat()}"
                    )
                    logger.debug(
                        f"Generated new item: tag_id={tag_id}, common_name={common_name}, rental_class_num={rental_class_num}, status=Sold"
                    )
        ...
        if mappings_dict is None:
            ...
            for um in user_mappings:
                ...
                logger.debug(
                    f"Added user mapping: rental_class_id={normalized_id}, short_common_name={um.short_common_name}"
                )
            cache_set(cache_key, mappings_dict, timeout=3600)
            logger.debug(f"Cached rental_class_mappings for common_name={common_name}")

        for item in synced_items:
            normalized_rental_class = normalize_rental_class_id(
                item["rental_class_num"]
            )
            mapping = mappings_dict.get(normalized_rental_class, {})
            item["subcategory"] = mapping.get("subcategory", "Unknown")
            item["short_common_name"] = mapping.get(
                "short_common_name", item["common_name"]
            )
            logger.debug(
                f"Mapping for item: tag_id={item['tag_id']}, rental_class_num={normalized_rental_class}, short_common_name={item['short_common_name']}"
            )
        ...
        all_items = [
            ...
            if item["tag_id"] not in {i["tag_id"] for i in synced_items}
        ]
        ...
        try:
            ...
            try:
                os.chown(
                    CSV_FILE_PATH,
                    pwd.getpwnam("tim").pw_uid,
                    grp.getgrnam("tim").gr_gid,
                )
            ...
            new_items_in_csv = [
                item for item in synced_items if item.get("is_new", False)
            ]
            existing_items_in_csv = [
                item for item in synced_items if not item.get("is_new", False)
            ]
            logger.info(
                f"AUDIT: CSV_SYNCED - total_items={len(all_items)}, new_items={len(new_items_in_csv)}, existing_items={len(existing_items_in_csv)}, operation=sync_to_pc, timestamp={datetime.now().isoformat()}"
            )
            for item in new_items_in_csv:
                logger.info(
                    f"AUDIT: CSV_ITEM_ADDED - tag_id={item['tag_id']}, common_name={item['common_name']}, status={item['status']}, ready_for_printing=true, timestamp={datetime.now().isoformat()}"
                )
        ...
        if new_items:
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not cleanup CSV after database failure: {csv_cleanup_error}"
                    )
                raise e

        logger.info(
            f"Successfully synced {len(synced_items)} items to CSV and database"
        )
...
def update_synced_status():
    ...
    try:
        logger.debug(
            "Entering /tab/3/update_synced_status endpoint at %s", datetime.now()
        )
        session = db.session()
        ...
        for attempt in range(max_lock_attempts):
            try:
                ...
                lock_acquired = True
                logger.debug(
                    f"Acquired lock for update_synced_status on attempt {attempt + 1}"
                )
                break
            except BlockingIOError:
                if attempt == max_lock_attempts - 1:
                    logger.warning(
                        f"Could not acquire lock for update_synced_status after {max_lock_attempts} attempts"
                    )
                    lock_file.close()
                    return (
                        jsonify(
                            {
                                "error": "System busy - another update operation is in progress. Please try again."
                            }
                        ),
                        429,
                    )
                logger.debug(
                    f"Update lock attempt {attempt + 1} failed, waiting {lock_wait_time}s"
                )
                time.sleep(lock_wait_time)
                lock_wait_time *= 1.5  # Exponential backoff
        ...
        if not os.access(CSV_FILE_PATH, os.R_OK | os.W_OK):
            ...
            return (
                jsonify(
                    {"error": f"CSV file {CSV_FILE_PATH} is not readable/writable"}
                ),
                500,
            )
            ...
            with open(CSV_FILE_PATH, "r") as csvfile:
                ...
                if not all(field in reader.fieldnames for field in required_fields):
                    ...
                    return (
                        jsonify(
                            {
                                "error": f"CSV file missing required fields: {required_fields}"
                            }
                        ),
                        400,
                    )
                ...
                for row in reader:
                    try:
                        ...
                        logger.error(
                            f"Missing required field in CSV row: {ke}, row: {row}"
                        )
                        return (
                            jsonify(
                                {"error": f"Missing required field in CSV row: {ke}"}
                            ),
                            400,
                        )
        except Exception as e:
        ...
        for i in range(0, len(tag_ids), batch_size):
            batch_tags = tag_ids[i : i + batch_size]
            updated = ItemMaster.query.filter(ItemMaster.tag_id.in_(batch_tags)).update(
                {
                    ItemMaster.status: "Ready to Rent",
                    ItemMaster.date_updated: datetime.now(),
                },
            ...
            for tag_id in batch_tags:
                logger.info(
                    f"AUDIT: STATUS_UPDATED - tag_id={tag_id}, old_status=Sold, new_status=Ready to Rent, operation=post_print_update, timestamp={datetime.now().isoformat()}"
                )
        ...
        for item in items_to_update:
            ...
            try:
                ...
                existing_items = api_client._make_request(
                    "14223767938169344381", params
                )
                if existing_items:
                    ...
                    new_item = {
                        ...
                        "bin_location": db_item.get(
                            "bin_location", item["bin_location"]
                        ),
                        "status": "Ready to Rent",
                        "date_last_scanned": datetime.now().strftime(
                            "%Y-%m-%d %H:%M:%S"
                        ),
                ...
                cache.delete(
                    f"tab3_items_{db_item.get('rental_class_num', '')}_{item['common_name']}"
                )
            except Exception as api_error:
                logger.error(
                    f"Error updating/inserting tag_id {tag_id} in API: {str(api_error)}",
                    exc_info=True,
                )
        ...
        api_failure_rate = (
            len(failed_items) / len(items_to_update) if items_to_update else 0
        )
        if api_failure_rate > 0.5:  # More than 50% API failures
            logger.error(
                f"API failure rate too high ({api_failure_rate:.1%}), aborting CSV clear to prevent data loss"
            )
            session.rollback()
            return (
                jsonify(
        ...
        if failed_items:
            logger.warning(
                f"API partial success: {api_success_count}/{len(items_to_update)} items updated, {len(failed_items)} failed: {failed_items}"
            )
        ...
        try:
            ...
            logger.info(
                f"AUDIT: CSV_CLEARED - operation=post_print_status_update, processed_tags={len(tag_ids)}, successful_api_updates={api_success_count}, failed_api_updates={len(failed_items)}, timestamp={datetime.now().isoformat()}"
            )
        ...
        session.commit()
        logger.info(
            f"Completed update_synced_status: Updated {updated_items} items and cleared CSV file"
        )
        return (
            jsonify(
                {
        ...
        logger.error(
            f"Uncaught exception in update_synced_status: {str(e)}", exc_info=True
        )
        if session:
            session.rollback()
        return jsonify({"error": str(e)}), 500
...
def update_status():
    ...
    try:
        ...
        if not tag_id or not new_status:
            logger.warning(
                f"Invalid input in update_status: tag_id={tag_id}, new_status={new_status}"
            )
...
def update_bin_location():
    ...
    try:
        logger.debug(
            "Entering /tab/3/update_bin_location endpoint at %s", datetime.now()
        )
        session = db.session()
        data = request.get_json()
        tag_id = data.get("tag_id")
        ...
        if new_bin_location not in valid_bin_locations and new_bin_location != "":
            ...
            return (
                jsonify(
                    {
                        "error": f'Bin location must be one of {", ".join(valid_bin_locations)} or empty'
                    }
                ),
                400,
            )
        ...
        logger.error(
            f"Uncaught exception in update_bin_location: {str(e)}", exc_info=True
        )
        if session:
            session.rollback()
        return jsonify({"error": str(e)}), 500
...
def get_pack_resale_common_names():
    ...
    try:
        logger.debug(
            "Entering /tab/3/pack_resale_common_names endpoint at %s", datetime.now()
        )
        ...
        cache_set(cache_key, response, timeout=60)
        logger.info(
            f"Fetched and cached {len(common_names)} common names from SeedRentalClass for pack/resale"
        )
        return jsonify(response), 200
    except Exception as e:
        logger.error(f"Error fetching common names: {str(e)}", exc_info=True)
...
def update_mappings():
    ...
    try:
        ...
        if not rental_class_id or not category or not subcategory:
            logger.warning(
                f"Invalid input in update_mappings: rental_class_id={rental_class_id}, category={category}, subcategory={subcategory}"
            )
            return (
                jsonify(
        ...
        existing_mapping = (
            ...
            .filter_by(rental_class_id=normalized_rental_class_id)
            .first()
        )
        ...
        if existing_mapping:
            ...
            existing_mapping.short_common_name = (
                short_common_name
                if short_common_name
                else existing_mapping.short_common_name
            )
            existing_mapping.updated_at = datetime.now(timezone.utc)
            logger.debug(
                f"Updated existing mapping: rental_class_id={normalized_rental_class_id}"
            )
        else:
            ...
            logger.debug(
                f"Created new mapping: rental_class_id={normalized_rental_class_id}"
            )
