...
def inventory_analytics_view():
    ...
    logger.info(
        "Loading Inventory & Analytics dashboard at %s",
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    )
...
def get_dashboard_summary():
    ...
    try:
        ...
        logger.info(
            f"Fetching dashboard summary with filters: store={store_filter}, type={type_filter}"
        )
        ...
        logger.info(
            f"Dashboard summary calculated: {summary['inventory_overview']['total_items']} total items, health score: {health_score}"
        )
...
def get_business_intelligence():
    ...
    try:
        ...
        type_filter = request.args.get("type", "all")
        logger.info(
            f"Fetching business intelligence with filters: store={store_filter}, type={type_filter}"
        )
        ...
        store_distribution = (
            ...
            .filter(*build_global_filters(store_filter, type_filter))
        ...
        type_distribution = (
            ...
            .group_by(ItemMaster.identifier_type)
            .all()
        )
        ...
        top_manufacturers = (
            base_query.filter(
                ItemMaster.manufacturer.isnot(None), ItemMaster.manufacturer != ""
            )
            .with_entities(
            ...
            .group_by(ItemMaster.manufacturer)
        ...
        if price_threshold_query.count() > 0:
            ...
            all_prices = [
                item.sell_price
                for item in price_threshold_query.all()
                if item.sell_price
            ]
            if all_prices:
                all_prices.sort()
        ...
        business_intel = {
                ...
                "distribution": [
                    {
                        ...
                        "avg_sell_price": (
                            decimal_to_float(store.avg_sell_price)
                            if store.avg_sell_price
                            else 0
                        ),
                        "total_turnover": (
                            decimal_to_float(store.total_turnover)
                            if store.total_turnover
                            else 0
                        ),
                    }
                ...
                "distribution": [
                    {
                        ...
                        "avg_price": (
                            decimal_to_float(type_dist.avg_price)
                            if type_dist.avg_price
                            else 0
                        ),
                    }
            ...
            "financial_insights": {
                ...
                "total_inventory_value": (
                    decimal_to_float(financial_data.total_inventory_value)
                    if financial_data.total_inventory_value
                    else 0
                ),
                "avg_sell_price": (
                    decimal_to_float(financial_data.avg_sell_price)
                    if financial_data.avg_sell_price
                    else 0
                ),
                "total_turnover_ytd": (
                    decimal_to_float(financial_data.total_turnover_ytd)
                    if financial_data.total_turnover_ytd
                    else 0
                ),
                "avg_turnover_ytd": (
                    decimal_to_float(financial_data.avg_turnover_ytd)
                    if financial_data.avg_turnover_ytd
                    else 0
                ),
                "total_repair_costs": (
                    decimal_to_float(financial_data.total_repair_costs)
                    if financial_data.total_repair_costs
                    else 0
                ),
                "avg_repair_cost": (
                    decimal_to_float(financial_data.avg_repair_cost)
                    if financial_data.avg_repair_cost
                    else 0
                ),
                "high_value_items": high_value_items,
                "high_value_threshold": (
                    decimal_to_float(price_percentile) if price_percentile else 0
                ),
            },
                ...
                "top_manufacturers": [
                    {
                        ...
                        "avg_price": (
                            decimal_to_float(mfr.avg_price) if mfr.avg_price else 0
                        ),
                    }
        ...
        logger.info(
            f"Business intelligence calculated for {business_intel['filter_context']['total_items_in_view']} items"
        )
...
def get_stale_items():
    ...
    try:
        session = db.session()
        logger.info(
            "🚀 Fetching stale items with GAME-CHANGING transaction integration!"
        )
        ...
        logger.info(
            f"🎯 Thresholds - Default: {default_days}, Resale: {resale_days}, Pack: {pack_days} days"
        )
        ...
        logger.info(
            f"🎉 Found {len(result_rows)} items using REVOLUTIONARY transaction analysis!"
        )
        ...
        for row in result_rows:
            ...
            is_previously_hidden = (
                row.touch_scan_count > 0  # Has touch scan activity
                and master_days_stale
                and master_days_stale
                > row.true_days_stale  # Transaction is newer than master
            )
            ...
            item_dict = {
                ...
                "notes": row.notes,
                "master_last_scan": (
                    row.master_last_scan.isoformat() if row.master_last_scan else None
                ),
                "category": row.category,
                "subcategory": row.subcategory,
                "true_last_activity": (
                    row.true_last_activity.isoformat()
                    if row.true_last_activity
                    else None
                ),
                ...
                "master_vs_transaction_days": (
                    {
                        ...
                        "difference": (
                            master_days_stale - row.true_days_stale
                            if master_days_stale
                            else 0
                        ),
                    }
                    if master_days_stale
        ...
        logger.info(
            f"🔥 GAME CHANGER: {touch_managed_count} items have Touch Scan management!"
        )
        logger.info(
            f"🚨 PREVIOUSLY HIDDEN: {previously_hidden_count} items were invisible in old analysis!"
        )
        ...
        return jsonify(
            {
                ...
                "enhanced_summary": {
                    ...
                    "items_with_transactions": sum(
                        1 for item in result if item["total_scan_count"] > 0
                    ),
                    "items_with_touch_scans": touch_managed_count,
                    "items_with_status_scans": sum(
                        1 for item in result if item["status_scan_count"] > 0
                    ),
                    "previously_hidden_by_old_analysis": previously_hidden_count,
                    "activity_breakdown": activity_stats,
                    "touch_managed_percentage": (
                        round((touch_managed_count / len(result) * 100), 1)
                        if result
                        else 0
                    ),
                },
        ...
        logger.error(
            f"💥 Error in revolutionary stale items analysis: {str(e)}", exc_info=True
        )
        return (
            jsonify({"error": f"Failed to fetch enhanced stale items: {str(e)}"}),
            500,
        )
    finally:
        if session:
...
def get_stale_items_simple():
    ...
    try:
        ...
        logger.info(
            f"Fetching stale items with thresholds - Default: {default_days}, Resale: {resale_days}, Pack: {pack_days} days"
        )
            ...
            items.append(
                {
                    ...
                    "subcategory": row.subcategory,
                    "master_last_scan": (
                        row.master_last_scan.isoformat()
                        if row.master_last_scan
                        else None
                    ),
        ...
        return jsonify(
            {
                ...
                "summary": {"total_stale_items": total_count, "showing": len(items)},
            }
        )
...
def get_usage_patterns():
    ...
    try:
        ...
        usage_patterns = (
            ...
            .group_by(
                UserRentalClassMapping.category, UserRentalClassMapping.subcategory
            )
            .having(
                func.count(func.distinct(ItemMaster.tag_id))
                > 5  # Only categories with 5+ items
            )
        ...
        for pattern in usage_patterns:
            activity_rate = (pattern.total_transactions or 0) / max(
                pattern.total_items, 1
            )
            patterns.append(
                {
        ...
        logger.info(
            f"Generated usage patterns for {len(patterns)} categories over {days_back} days"
        )
        return jsonify(
            {
...
def get_data_discrepancies():
    ...
    try:
        ...
        no_transactions = (
            ...
            .outerjoin(Transaction, ItemMaster.tag_id == Transaction.tag_id)
            .filter(Transaction.tag_id.is_(None))
            .limit(50)
            .all()
        )
        ...
        orphaned_transactions = (
            ...
            .outerjoin(ItemMaster, Transaction.tag_id == ItemMaster.tag_id)
            .filter(ItemMaster.tag_id.is_(None))
            ...
            "outdated_master_records": [
                {
                    ...
                    "transaction_last_scan": (
                        item.latest_transaction.isoformat()
                    ...
                    "days_difference": (
                        (
                            ...
                            - (item.date_last_scanned or item.latest_transaction)
                        ).days
                        if item.latest_transaction
                        else None
                    ),
            ...
            "items_without_transactions": [
                {
                    ...
                    "status": item.status,
                    "created_date": (
                        item.date_created.isoformat() if item.date_created else None
                    ),
                }
            ...
            "orphaned_transactions": [
                {
                    "tag_id": trans.tag_id,
                    "latest_scan": (
                        trans.latest_scan.isoformat() if trans.latest_scan else None
                    ),
        ...
        logger.info(
            f"Found data discrepancies: {len(discrepancies['outdated_master_records'])} outdated, {len(discrepancies['items_without_transactions'])} without transactions, {len(discrepancies['orphaned_transactions'])} orphaned"
        )
...
def check_utilization_patterns(session):
    ...
    try:
        ...
        category_stats = (
            ...
            .having(func.count(ItemMaster.tag_id) > 10)
            .all()
        )  # Only categories with 10+ items
...
def get_alert_config(session):
    ...
    try:
        config_record = (
            session.query(InventoryConfig)
            .filter(InventoryConfig.config_key == "alert_thresholds")
            .first()
        )
...
def get_configuration():
    ...
    try:
        ...
        business_config_record = (
            session.query(InventoryConfig)
            .filter(InventoryConfig.config_key == "business_rules")
            .first()
        )

        business_rules = (
            business_config_record.config_value
        ...
        dashboard_config_record = (
            session.query(InventoryConfig)
            .filter(InventoryConfig.config_key == "dashboard_settings")
            .first()
        )

        dashboard_settings = (
            dashboard_config_record.config_value
