...
# Version marker
logger.info(
    "Deployed tabs.py version: 2025-06-27-v5 at %s",
...
def tab_view(tab_num):
    logger.debug(
        f"Routing request for /tab/{tab_num} at %s",
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    )
    if tab_num == 1:
        logger.debug(
            "Redirecting to tab1.tab1_view at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        return redirect(url_for("tab1.tab1_view"))
    elif tab_num == 2:
        logger.debug(
            "Redirecting to tab2.tab2_view at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        return redirect(url_for("tab2.tab2_view"))
    elif tab_num == 3:
        logger.debug(
            "Redirecting to tab3.tab3_view at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        return redirect(url_for("tab3.tab3_view"))
    elif tab_num == 4:
        logger.debug(
            "Redirecting to tab4.tab4_view at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        return redirect(url_for("tab4.tab4_view"))
    elif tab_num == 5:
        logger.debug(
            "Redirecting to tab5.tab5_view at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        return redirect(url_for("tab5.tab5_view"))
    elif tab_num == 7:
        logger.debug(
            "Redirecting to tab7.tab7_view at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        return redirect(url_for("tab7.tab7_view"))
    else:
        logger.warning(
            f"Tab {tab_num} not implemented at %s",
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
...
def tab_categories(tab_num):
    logger.debug(
        f"Routing categories request for /tab/{tab_num} at %s",
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    )
    if tab_num == 1:
...
def subcat_data(tab_num):
    logger.debug(
        f"Routing subcat_data request for /tab/{tab_num} at %s",
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    )
    if tab_num == 1:
...
def common_names(tab_num):
    logger.debug(
        f"Routing common_names request for /tab/{tab_num} at %s",
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    )
    if tab_num == 1:
...
def tab_data(tab_num):
    logger.debug(
        f"Routing data request for /tab/{tab_num} at %s",
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    )
    if tab_num == 1:
