...
enhanced_analytics_bp = Blueprint(
    "enhanced_analytics", __name__, url_prefix="/api/enhanced"
)
...
def get_enhanced_kpis():
    ...
    try:
        ...
        for i in range(min(weeks, 12)):
            ...
            week_revenue = (
                db.session.query(func.sum(PayrollTrends.total_revenue))
                .filter(
                    and_(
        ...
        try:
            active_alerts_result = db.session.execute(
                text(
                    "SELECT COUNT(*) FROM inventory_health_alerts WHERE status = 'active'"
                )
            ).scalar()
...
def get_inventory_distribution():
    ...
    try:
        ...
        type_distribution = (
            base_query.with_entities(
                ItemMaster.identifier_type, func.count(ItemMaster.tag_id).label("count")
            )
        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    "type_distribution": {"labels": type_labels, "values": type_values},
                    "totals": {
                        "total_items": total_items,
...
def get_financial_metrics():
    ...
    try:
        ...
        high_value_items = (
            base_query.filter(ItemMaster.sell_price >= 1000).count()
        ...
        manufacturer_data = (
            ...
            .filter(ItemMaster.manufacturer.isnot(None))
        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    "manufacturer_insights": {"top_manufacturers": top_manufacturers},
                    "coverage_metrics": {
                        "data_completeness": (
                            items_with_price / max(base_query.count(), 1)
                        )
                        * 100,
                        "high_value_percentage": (
                            high_value_items / max(base_query.count(), 1)
                        )
                        * 100,
                    },
                },
        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    "summary": {
                        ...
                        "high_utilization_categories": len(
                            [
                                c
                                for c in category_utilization
                                if c["utilization_rate"] >= 80
                            ]
                        ),
                        "low_utilization_categories": len(
                            [
                                c
                                for c in category_utilization
                                if c["utilization_rate"] <= 40
                            ]
                        ),
                    },
                },
