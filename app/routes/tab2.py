...
def tab2_view():
    ...
    try:
        ...
        logger.debug(
            f"All distinct contract numbers in ItemMaster: {[c[0] for c in all_contracts]}"
        )

        # Debug: Check statuses
        status_counts = (
            session.query(ItemMaster.status, func.count(ItemMaster.tag_id))
        ...
        logger.debug(
            f"Base query results: {[(r.last_contract_num, r.total_items) for r in base_results]}"
        )
        ...
        logger.info(
            f"Raw contracts query result: {[(c.last_contract_num, c.total_items) for c in contracts_query_results]}"
        )
        ...
        for contract_number, total_items in contracts_query_results:
            ...
            is_stale = (
                ...
                else (now - scan_datetime > timedelta(days=12))
            )
            logger.debug(
        ...
        logger.info(
            f"Fetched {len(contracts)} contracts for tab2: {[c['contract_number'] for c in contracts]}"
        )
        current_app.logger.info(
            f"Fetched {len(contracts)} contracts for tab2: {[c['contract_number'] for c in contracts]}"
        )
        ...
        for handler in current_app.logger.handlers:
            handler.flush()
        return render_template(
            "tab2.html", contracts=[], error=str(e), cache_bust=int(time())
        )
    finally:
        if session:
            try:
...
def sort_contracts():
    ...
    try:
        ...
        if sort_column == "contract_number":
            contracts_query = contracts_query.order_by(
                sort_direction(ItemMaster.last_contract_num)
            )
        elif sort_column == "client_name":
            contracts_query = contracts_query.order_by(
                sort_direction(Transaction.client_name)
            )
        elif sort_column == "scan_date":
            contracts_query = contracts_query.order_by(
                sort_direction(func.max(Transaction.scan_date))
            )

        contracts_query_results = contracts_query.all()
        logger.info(
            f"Sorted contracts query result: {[(c.last_contract_num, c.total_items) for c in contracts_query_results]}"
        )
        current_app.logger.info(
            f"Sorted contracts query result: {[(c.last_contract_num, c.total_items) for c in contracts_query_results]}"
        )
...
def tab2_common_names():
    ...
    logger.info(
        f"Fetching common names for contract_number={contract_number}, page={page}, sort={sort}"
    )
    current_app.logger.info(
        f"Fetching common names for contract_number={contract_number}, page={page}, sort={sort}"
    )
    ...
    if not contract_number:
        ...
        current_app.logger.error(
            "Missing required parameter: contract_number is required"
        )
    ...
    try:
        ...
        common_names_query = (
            session.query(ItemMaster.common_name, on_contracts)
            .filter(
                ItemMaster.last_contract_num == contract_number,
                func.lower(ItemMaster.status).in_(["on rent", "delivered"]),
            )
        ...
        if sort == "name_asc":
            ...
            common_names_query = common_names_query.order_by(
                asc("total_items_inventory")
            )
        elif sort == "total_items_inventory_desc":
            common_names_query = common_names_query.order_by(
                desc("total_items_inventory")
            )
        ...
        logger.debug(
            f"Common names for contract {contract_number}: {[(name, count) for name, count in common_names_all]}"
        )
        ...
        if sort in ["total_items_inventory_asc", "total_items_inventory_desc"]:
            ...
            common_names.sort(
                key=lambda x: x[sort_field], reverse=(sort_direction == "desc")
            )
        ...
        logger.info(
            f"Returning {len(paginated_common_names)} common names for contract {contract_number}"
        )
        current_app.logger.info(
            f"Returning {len(paginated_common_names)} common names for contract {contract_number}"
        )
        ...
        logger.error(
            f"Error fetching common names for contract {contract_number}: {str(e)}",
            exc_info=True,
        )
        current_app.logger.error(
            f"Error fetching common names for contract {contract_number}: {str(e)}",
...
def update_status():
    ...
    try:
        ...
        if session:
            session.rollback()
        logger.error(
            f"Error updating status for tag {tag_id if 'tag_id' in locals() else ''}: {str(e)}",
            exc_info=True,
        )
    ...
    try:
        ...
        for item in items:
            ...
            try:
                ...
                logger.error(
                    f"Failed to update API status for tag_id {item.tag_id}: {str(e)}",
                    exc_info=True,
                )
        ...
        if session:
            session.rollback()
        logger.error(
            f"Error bulk updating status for contract {contract_number if 'contract_number' in locals() else ''}: {str(e)}",
            exc_info=True,
        )
...
def tab2_data():
    ...
    if not contract_number or not common_name:
        logger.error(
            "Missing required parameters: contract number and common name are required"
        )
        current_app.logger.error(
            "Missing required parameters: contract number and common name are required"
        )
    ...
    try:
        ...
        if sort == "tag_id_asc":
            ...
            query = query.order_by(
                asc(func.lower(func.coalesce(ItemMaster.bin_location, "")))
            )
        elif sort == "bin_location_desc":
            query = query.order_by(
                desc(func.lower(func.coalesce(ItemMaster.bin_location, "")))
            )
            ...
            query = query.order_by(
                asc(func.lower(func.coalesce(ItemMaster.last_contract_num, "")))
            )
        elif sort == "last_contract_desc":
            query = query.order_by(
                desc(func.lower(func.coalesce(ItemMaster.last_contract_num, "")))
            )
        ...
        logger.debug(
            f"Items for contract {contract_number}, common_name {common_name}: {len(items_data)} items"
        )
        current_app.logger.debug(
            f"Items for contract {contract_number}, common_name {common_name}: {len(items_data)} items"
        )
        ...
        logger.error(
            f"Error fetching items for contract {contract_number}, common_name {common_name}: {str(e)}",
            exc_info=True,
        )
        current_app.logger.error(
            f"Error fetching items for contract {contract_number}, common_name {common_name}: {str(e)}",
...
def full_items_by_rental_class():
    ...
    if not contract_number or not common_name:
        ...
        current_app.logger.error(
            "Category (contract_number) and common name are required"
        )
        return (
            jsonify(
    ...
    try:
        ...
        logger.error(
            f"Error fetching full items for contract {contract_number}, common_name {common_name}: {str(e)}",
            exc_info=True,
        )
        current_app.logger.error(
            f"Error fetching full items for contract {contract_number}, common_name {common_name}: {str(e)}",
