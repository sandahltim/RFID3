...
def get_data_availability():
    ...
    try:
        ...
        earliest = session.query(func.min(PayrollTrends.week_ending)).filter(
            PayrollTrends.total_revenue > 0
        ).scalar()
        
        latest = session.query(func.max(PayrollTrends.week_ending)).filter(
            PayrollTrends.total_revenue > 0
        ).scalar()
        
        total_records = session.query(func.count(PayrollTrends.id)).filter(
            PayrollTrends.total_revenue > 0
        ).scalar()
        ...
        return jsonify({'error': 'Failed to get data availability', 'details': str(e)}), 500
    finally:
        if session:
            session.close()
    ...
    try:
        ...
        for row in results:
            ...
            data_point = {
                ...
                'profit_margin': round(((float(row.total_revenue or 0) - float(row.payroll_cost or 0)) / float(row.total_revenue or 1)) * 100, 2)
            }
            ...
            weekly_aggregates[week_key]['total_rental_revenue'] += data_point['rental_revenue']
        ...
        for week_data in weekly_aggregates.values():
            ...
            aggregated_trends.append({
                ...
                'profit_margin': round(((total_rev - total_pay) / total_rev) * 100, 2) if total_rev > 0 else 0,
                'efficiency': round((total_pay / total_rev) * 100, 2) if total_rev > 0 else 0,
                'revenue_per_hour': round(total_rev / week_data['total_hours'], 2) if week_data['total_hours'] > 0 else 0,
        ...
        return jsonify({
            ...
            'date_range': {
                'start': aggregated_trends[0]['week'] if aggregated_trends else None,
...
def get_growth_analysis():
    ...
    try:
        ...
        growth_analysis = {
            'week_over_week': {
                'revenue_growth': calculate_growth(current_week.revenue, prev_week.revenue),
                'payroll_growth': calculate_growth(current_week.payroll, prev_week.payroll),
                'efficiency_change': calculate_growth(current_week.efficiency, prev_week.efficiency),
                'current_revenue': float(current_week.revenue or 0),
                'prev_revenue': float(prev_week.revenue or 0)
            },
            'month_over_month': {
                'revenue_growth': calculate_growth(current_month.revenue, prev_month.revenue),
                'payroll_growth': calculate_growth(current_month.payroll, prev_month.payroll),
                'efficiency_change': calculate_growth(current_month.efficiency, prev_month.efficiency),
                'current_revenue': float(current_month.revenue or 0),
                'prev_revenue': float(prev_month.revenue or 0)
            },
            'year_over_year': {
                'revenue_growth': calculate_growth(current_year.revenue, prev_year.revenue),
                'payroll_growth': calculate_growth(current_year.payroll, prev_year.payroll),
                'efficiency_change': calculate_growth(current_year.efficiency, prev_year.efficiency),
        ...
        revenue_velocity = {
            'wow_acceleration': growth_analysis['week_over_week']['revenue_growth'] - growth_analysis['month_over_month']['revenue_growth'],
            'mom_acceleration': growth_analysis['month_over_month']['revenue_growth'] - growth_analysis['year_over_year']['revenue_growth']
        }
...
def get_forecasting():
    ...
    try:
        ...
        logger.info(f"Generating {forecast_weeks}-week forecast for store: {store_filter}")
        ...
        query = query.group_by(PayrollTrends.week_ending).order_by(PayrollTrends.week_ending.desc()).limit(52)
        ...
        for week in range(1, forecast_weeks + 1):
            ...
            seasonal_factor = 1 + 0.1 * (week % 4 - 2) / 10  # Simple quarterly seasonality
            
            forecast_revenue = (revenue_slope * data_point + revenue_intercept) * seasonal_factor
            forecast_payroll = (payroll_slope * data_point + payroll_intercept) * seasonal_factor
            
            # Calculate confidence intervals (simple approach)
            revenue_std = (sum([(r - sum(revenues)/len(revenues))**2 for r in revenues]) / len(revenues)) ** 0.5
            ...
            forecasts.append({
                ...
                'forecast_profit': round(max(0, forecast_revenue - forecast_payroll), 2),
                'confidence_upper': round(forecast_revenue + confidence_interval, 2),
                'confidence_lower': round(max(0, forecast_revenue - confidence_interval), 2),
                'trend_strength': abs(revenue_slope) / (sum(revenues)/len(revenues)) if revenues else 0
            })
...
def get_store_benchmarking():
    ...
    try:
        ...
        company_benchmarks = {
            'avg_weekly_revenue': total_company_revenue / (len(store_metrics) * period_weeks) if store_metrics else 0,
            'avg_profit_margin': ((total_company_revenue - total_company_payroll) / total_company_revenue * 100) if total_company_revenue else 0,
            'avg_revenue_per_hour': total_company_revenue / total_company_hours if total_company_hours else 0,
            'avg_efficiency_ratio': (total_company_payroll / total_company_revenue * 100) if total_company_revenue else 0
        }
        ...
        for store in store_metrics:
            ...
            revenue_vs_avg = (revenue / (period_weeks or 1)) / company_benchmarks['avg_weekly_revenue'] if company_benchmarks['avg_weekly_revenue'] else 0
            ...
            performance_scores = {
                ...
                'efficiency_score': min(100, max(0, 100 - float(store.avg_efficiency or 0))),
                'consistency_score': min(100, max(0, 100 - (float(store.revenue_volatility or 0) / (revenue / period_weeks) * 100))) if revenue > 0 else 0
            }
...
def calculate_comparison_metrics(session, store_filter, base_start, base_end, comp_start, comp_end):
    ...
    def get_metrics(start_date, end_date):
        ...
        return {
            ...
            'profit_margin': ((float(result.revenue or 0) - float(result.payroll or 0)) / float(result.revenue or 1)) * 100,
            'contracts': int(scorecard.contracts or 0) if scorecard else 0,
            'ar_aging': float(scorecard.ar_aging or 0) if scorecard else 0,
            'revenue_per_hour': float(result.revenue or 0) / float(result.hours or 1) if result.hours else 0
        }
    ...
    return {
        ...
        'changes': {
            ...
            'rental_revenue': calc_change(base_metrics['rental_revenue'], comp_metrics['rental_revenue']),
            'payroll': calc_change(base_metrics['payroll'], comp_metrics['payroll']),
            'profit': calc_change(base_metrics['profit'], comp_metrics['profit']),
            'profit_margin': base_metrics['profit_margin'] - comp_metrics['profit_margin'],
            'efficiency': calc_change(base_metrics['efficiency'], comp_metrics['efficiency']),
            'contracts': calc_change(base_metrics['contracts'], comp_metrics['contracts']),
            'revenue_per_hour': calc_change(base_metrics['revenue_per_hour'], comp_metrics['revenue_per_hour'])
        }
    }
...
def get_period_comparison_legacy():
    ...
    try:
        ...
        logger.info(f"Period comparison: type={comparison_type}, base={base_start} to {base_end}")
        ...
        if comparison_type == 'wow':
            ...
            if base_end.month == 1:
                ...
                comp_end = base_end.replace(month=base_end.month - 1, day=min(base_end.day, last_day))
            ...
            if not comp_start_str or not comp_end_str:
                return jsonify({'error': 'Comparison dates required for custom comparison'}), 400
...
def get_data_availability_legacy():
    ...
    try:
        ...
        result = session.query(
            ...
            func.count(func.distinct(PayrollTrends.week_ending)).label('total_weeks')
        ).filter(
            PayrollTrends.total_revenue > 0  # Exclude zero-value future dates
        ).first()
        
        # Get store-specific availability
        store_data = session.query(
            PayrollTrends.store_id,
        ...
        for store in store_data:
            stores_availability.append({
                'store_id': store.store_id,
                'store_name': STORE_MAPPING.get(store.store_id, store.store_id),
                'earliest_date': store.earliest.isoformat() if store.earliest else None,
...
def get_trending_analysis():
    ...
    try:
        ...
        logger.info(f"Trending analysis: metrics={metrics}, dates={start_date} to {end_date}")
        ...
        query = query.group_by(PayrollTrends.week_ending).order_by(PayrollTrends.week_ending)
        ...
        for row in results:
            ...
            if 'profit_margin' in metrics:
                data_point['profit_margin'] = ((revenue - payroll) / revenue * 100) if revenue else 0
        ...
        return jsonify({
            ...
            'metrics_available': ['revenue', 'rental_revenue', 'payroll', 'profit', 
                                 'profit_margin', 'efficiency', 'revenue_per_hour', 'hours'],
            'date_range': {
...
def get_period_comparison():
    ...
    try:
        ...
        if not current_start or not current_end:
            return jsonify({'error': 'current_start and current_end dates required'}), 400
        ...
        def calculate_totals(data):
            if not data:
                return {'revenue': 0, 'payroll': 0, 'hours': 0, 'revenue_per_hour': 0, 'profit': 0, 'weeks': 0}
            ...
            totals['revenue_per_hour'] = totals['revenue'] / totals['hours'] if totals['hours'] > 0 else 0
            totals['profit_margin'] = (totals['profit'] / totals['revenue'] * 100) if totals['revenue'] > 0 else 0
        ...
        comparison_result = {
            ...
            'changes': {
                'revenue': calc_change(current_totals['revenue'], compare_totals['revenue']),
                'payroll': calc_change(current_totals['payroll'], compare_totals['payroll']),
                'profit': calc_change(current_totals['profit'], compare_totals['profit']),
                'profit_margin': calc_change(current_totals['profit_margin'], compare_totals['profit_margin']),
                'revenue_per_hour': calc_change(current_totals['revenue_per_hour'], compare_totals['revenue_per_hour']),
                'hours': calc_change(current_totals['hours'], compare_totals['hours'])
            },
            'summary': {
                'total_revenue_change': current_totals['revenue'] - compare_totals['revenue'],
                'total_profit_change': current_totals['profit'] - compare_totals['profit'],
...
logger.info("Executive Dashboard Enhanced v3 - YoY/MoM/WoW Comparisons - Deployed %s", datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
