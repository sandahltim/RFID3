...
def get_tag_data(filter_tag_id="", filter_common_name="", filter_category="", filter_status=""):
    logging.debug(f"Fetching tag data with filters: tag_id={filter_tag_id}, common_name={filter_common_name}, category={filter_category}, status={filter_status}")
    try:
        with DatabaseConnection() as conn:
            ...
            logging.debug(f"Fetched {len(tags)} tags from id_rfidtag")
        ...
        for tag in tag_list:
            ...
            if item:
                if item['status'] in ['On Rent', 'Delivered'] and tag['status'] != 'out/used':
                    ...
                    logging.debug(f"Updated tag {tag['tag_id']} to out/used")
                    ...
                    logging.debug(f"Updated tag {tag['tag_id']} to active")

        return tag_list
    except Exception as e:
        logging.error(f"Error in get_tag_data: {e}")
        return []
...
def show_tab7():
    ...
    try:
        ...
        logging.debug(f"Rendering tab7 with {len(tags)} tags")
        ...
        if not tags:
            logging.warning("No tags found in id_rfidtag table")
...
def tab7_data():
    ...
    try:
        ...
        tags = get_tag_data(filter_tag_id, filter_common_name, filter_category, filter_status)
        logging.debug(f"Returning {len(tags)} tags for /tab7/data")
...
def print_tags():
    ...
    try:
        with DatabaseConnection() as conn:
            ...
            """).fetchall()
            logging.debug(f"Found {len(tags)} tags to print")
